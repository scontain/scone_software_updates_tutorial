...
release_version_flag="--release-version"
  ...
  case $1 in
    ...
    ${release_version_flag})  
      ...
      if [ ! -n "${RELEASE_VERSION}" ]; then
        usage
        error_exit "Error: The release version '$RELEASE_VERSION' is invalid."
      fi
      shift # past argument
      shift || true # past value
      ;;
...
if [ -z "$APP_NAMESPACE" ] ; then
    ...
    echo "Creating new CAS Namespace: $APP_NAMESPACE"
    echo " - release is $RELEASE_VERSION"
else
    RELEASE_VERSION=$((RELEASE_VERSION+1))
    echo "CAS Namespace is already defined: $APP_NAMESPACE"
    echo " - Releasing version $RELEASE_VERSION"
fi
# Toggle between version 1 and 2
ALTERNATIVE=$((((1+RELEASE_VERSION) % 2) + 1))


echo -e "export APP_NAMESPACE=\"$APP_NAMESPACE\"\n" > release.sh  
echo -e "export RELEASE_VERSION=$RELEASE_VERSION\n"  >> release.sh 
...
echo -e  "${BLUE}build service image:${NC} service${ALTERNATIVE}.yaml"
echo -e  "${BLUE} - if the push fails, add --no-push to avoid pushing the image, or${NC}"
echo -e  "${BLUE}   change in file '${ORANGE}service$ALTERNATIVE.yaml${BLUE}' field '${ORANGE}build.to${BLUE}' to a container repo to which you have permission to push.${NC}"

SCONE="\$SCONE" envsubst < service$ALTERNATIVE.yaml.template > service$ALTERNATIVE.yaml

sconectl apply -f service$ALTERNATIVE.yaml $verbose $debug --set-version ${VERSION}
...
echo -e "  - update the namespace '${ORANGE}policy.namespace${NC}' to a unique name in '${ORANGE}mesh${ALTERNATIVE}.yaml${NC}'"

export CAS_URL="${CAS}.${CAS_NAMESPACE}"
SCONE="\$SCONE" envsubst < mesh$ALTERNATIVE.yaml.template > mesh$ALTERNATIVE.yaml

sconectl apply -f mesh$ALTERNATIVE.yaml --release "$RELEASE" --release-version $ALTERNATIVE $verbose $debug  --set-version ${VERSION}


echo -e "${BLUE}install application:${NC} helm upgrade ${namespace_args} ${RELEASE} target/helm/"
